...
void mc_interface_init(bool reset_conf)
{
	...
	{
	case MOTOR_TYPE_DC:
		mcpwm_dc_init(&motor_now()->m_conf);
		break;
...
void mc_interface_set_configuration(mc_configuration *configuration)
{
	...
	{
	case MOTOR_TYPE_DC:
		mcpwm_dc_set_configuration(&motor->m_conf);
		break;
...
void mc_interface_set_duty(float dutyCycle)
{
	...
	{
	case MOTOR_TYPE_DC:
		mcpwm_dc_set_duty(DIR_MULT * dutyCycle);
		break;
...
void mc_interface_set_duty_noramp(float dutyCycle)
{
	...
	{
	case MOTOR_TYPE_DC:
		mcpwm_dc_set_duty_noramp(DIR_MULT * dutyCycle);
		break;
...
void mc_interface_set_pid_speed(float rpm)
{
	...
	{
	case MOTOR_TYPE_DC:
		mcpwm_dc_set_pid_speed(DIR_MULT * rpm);
		break;
...
void mc_interface_set_current(float current)
{
	...
	{
	case MOTOR_TYPE_DC:
		mcpwm_dc_set_current(DIR_MULT * current);
		break;
...
void mc_interface_set_handbrake(float current)
{
	...
	{
	case MOTOR_TYPE_DC:
		mcpwm_dc_set_parking_brake(current > 0);
		break;
...
bool mc_interface_wait_for_motor_release(float timeout)
{
	...
	case MOTOR_TYPE_DC:
		...
		{
			if (mcpwm_dc_get_state() == MC_STATE_OFF)
			{
				res = true;
				break;
			}
...
float mc_interface_get_duty_cycle_now(void)
{
	...
	{
	case MOTOR_TYPE_DC:
		ret = mcpwm_dc_get_duty_cycle_now();
		break;
...
float mc_interface_get_sampling_frequency_now(void)
{
	...
	{
	case MOTOR_TYPE_DC:
		ret = mcpwm_dc_get_switching_frequency_now();
		break;
...
float mc_interface_get_tot_current_filtered(void)
{
	...
	{
	case MOTOR_TYPE_DC:
		ret = mcpwm_dc_get_tot_current_filtered();
		break;
...
void mc_interface_mc_timer_isr(bool is_second_motor, float dt)
{
	...
	if (voltage_diff_now > 1.0e-3)
	{
		...
		{
			mc_interface_fault_stop(input_voltage < conf_now->l_min_vin ? FAULT_CODE_UNDER_VOLTAGE : FAULT_CODE_OVER_VOLTAGE, is_second_motor, true);
...
static void update_override_limits(volatile motor_if_state_t *motor, volatile mc_configuration *conf)
{
	...
	case TEMP_SENSOR_NTCX:
		temp_motor = is_motor_1 ? NTCX_TEMP_MOTOR(conf->m_ntcx_ptcx_res, conf->m_ntc_motor_beta, conf->m_ntcx_ptcx_temp_base) : NTCX_TEMP_MOTOR_2(conf->m_ntcx_ptcx_res, conf->m_ntc_motor_beta, conf->m_ntcx_ptcx_temp_base);
		break;

	case TEMP_SENSOR_PTCX:
		temp_motor = is_motor_1 ? PTC_TEMP_MOTOR(conf->m_ntcx_ptcx_res, conf->m_ptc_motor_coeff, conf->m_ntcx_ptcx_temp_base) : PTC_TEMP_MOTOR_2(conf->m_ntcx_ptcx_res, conf->m_ptc_motor_coeff, conf->m_ntcx_ptcx_temp_base);
		break;
